...
class StaffCabinetForm(QWidget):
    ...
    def init_ui(self):
        ...
        self.btn_export.setObjectName("btn-secondary")
    ...
    def generate_receipt_pdf(self, sale_id: int, client_id: int, car_id: int):
        try:
            font_path = "DejaVuSans.ttf"
            if not os.path.exists(font_path):
                raise FileNotFoundError(f"Шрифт {font_path} не найден. Скачайте его и поместите в папку проекта.")
            ...
            cur.execute("""
                            SELECT p.Data_prodazhi,
                                CONCAT(c.Familia, ' ', c.Imya, ' ',  c.Otchestvo) AS client_fio,
                                c.Nomer_telephona AS client_phone,
                                m.Nazvanie AS marka,
                                a.Model,
                                a.Complectacia,
                                a.Stoimost        FROM prodazhi_automobiley p
                            JOIN Client c ON p.ID_clienta = c.ID
                            JOIN automobile a ON p.ID_automobilya = a.id
                            JOIN Marka m ON a.Marka = m.id
                            WHERE p.ID = %s
                        """, (sale_id,))
            result = cur.fetchone()
            conn.close()
            ...
            if not result:
                raise Exception("Данные о продаже не найдены")

            data_prodazhi, client_fio, client_phone, marka, model, complectacia, stoimost = result

            filename = f"Чек_продажи_№{sale_id}.pdf"
            filepath = os.path.join("C:/Users/Я/Downloads/Курсовой проект/", filename)
            ...
            c.setFont("DejaVu", 16)
            c.drawCentredString(width / 2, height - 50, "ЧЕК ПРОДАЖИ")
            c.setFont("DejaVu", 12)
            c.drawCentredString(width / 2, height - 70, f"Номер чека: {sale_id}")
            c.drawString(50, height - 100, f"Дата: {data_prodazhi.strftime('%d.%m.%Y %H:%M')}")

            c.setFont("DejaVu", 12)
            c.drawString(50, height - 130, "Клиент:")
            c.setFont("DejaVu", 10)
            c.drawString(50, height - 150, f"ФИО: {client_fio}")
            c.drawString(50, height - 170, f"Телефон: {client_phone}")

            c.setFont("DejaVu", 12)
            c.drawString(50, height - 200, "Автомобиль:")
            c.setFont("DejaVu", 10)
            c.drawString(50, height - 220, f"Марка: {marka}")
            c.drawString(50, height - 240, f"Модель: {model}")
            c.drawString(50, height - 260, f"Комплектация: {complectacia}")
            c.drawString(50, height - 280, f"Стоимость: {stoimost:,} ₽".replace(',', ' '))

            c.line(50, height - 300, width - 50, height - 300)
            c.setFont("DejaVu", 14)
            c.drawString(50, height - 320, f"ИТОГО: {stoimost:,} ₽".replace(',', ' '))

            c.setFont("DejaVu", 10)
            c.drawString(50, height - 350, "Автомобиль продан. Претензий нет.")
            c.drawString(width - 150, height - 350, "Подпись клиента: __________")
            ...
            QMessageBox.critical(self, "Ошибка PDF", f"Не удалось создать чек:\n{e}")
            return None
    ...
    def sell_car(self):
        ...
        try:
            ...
            conn.commit()
            conn.close()
            pdf_path = self.generate_receipt_pdf(new_id, client_id, car_id)

            if pdf_path:
                msg = f"Продажа оформлена! Номер продажи: {new_id}\nЧек сохранён: {pdf_path}"
            else:
...
class AvtoForm(QWidget):
    def __init__(self):
        ...
        margin = 30
        table_width = int(screen.width() * 0.7)
        table_height = int(screen.height() * 0.7)
        self.ui.tableView.setGeometry(margin, margin, table_width, table_height)

        photo_x = margin + table_width + 20
        self.ui.photo_label.setGeometry(photo_x, margin, 250, 250)

        search_y = margin + table_height + 20
        self.ui.lineEdit.setGeometry(margin, search_y, 300, 50)
        self.ui.pushButton.setGeometry(margin + 320, search_y, 120, 50)
        self.ui.pushButton_2.setGeometry(margin + 450, search_y, 120, 50)
        self.ui.pushButton_3.setGeometry(screen.width() - 160, search_y, 150, 50)
